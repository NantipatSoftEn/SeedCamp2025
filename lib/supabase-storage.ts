...
export class SupabaseStorageService {
  ...
  async uploadPaymentSlip(
    ...
    try {
      ...
      if (!bucketReady) {
        throw new Error(
          "Could not create or access payment slips bucket. Please check Supabase permissions."
        );
      }
      ...
      if (personCheckError || !personCheck) {
        console.error(
          "❌ Person not found in seedcamp_people:",
          personCheckError
        );
      ...
      try {
        ...
        if (dbError) {
          ...
          if (dbError.message.includes("row-level security")) {
            ...
            throw new Error(
              `Foreign key constraint error: ${dbError.message}\n\nPerson ID ${personId} may not exist.`
            );
          } else {
            throw new Error(
              `Failed to save payment slip record: ${dbError.message}`
            );
          }
        }
        ...
        console.warn(
          "⚠️ Could not save to payment_slips table, continuing with person update:",
          dbError
        );
      ...
      if (updateError) {
        ...
        if (paymentSlipData) {
          await this.supabase
            .from("payment_slips")
            .delete()
            .eq("id", paymentSlipData.id);
        }

        throw new Error(
          `Failed to update person payment status: ${updateError.message}`
        );
      } else {
        console.log(
          "✅ Payment status updated to 'paid' and payment_slip path saved:",
          filePath
        );
  ...
  async deletePaymentSlip(
    ...
    try {
      ...
      if (personId) {
        ...
        if (updateError) {
          console.error(
            "❌ Failed to update payment status after deletion:",
            updateError
          );
          throw new Error(
            `Failed to update person payment status: ${updateError.message}`
          ...
          console.log(
            "✅ Payment status updated to 'unpaid' and payment_slip cleared"
          );
        }
        ...
        if (dbError) {
          console.warn(
            "⚠️ Could not delete payment slip record:",
            dbError.message
          );
        } else {
  ...
  async deleteAllPaymentSlipsForPerson(personId: string): Promise<boolean> {
    try {
      ...
      if (!paymentSlips || paymentSlips.length === 0) {
        ...
        console.log(
          `📁 Found ${paymentSlips.length} payment slip(s) to delete:`,
          paymentSlips
        );
        ...
        if (filePaths.length > 0) {
          ...
          if (storageError) {
            console.warn(
              "⚠️ Could not delete some files from storage:",
        ...
        if (dbError) {
          console.error(
            "❌ Could not delete payment slip records:",
            dbError.message
          );
        } else {
      ...
      console.log(
        "✅ Successfully deleted all payment slips for person:",
        personId
      );
      return true;
    } catch (error) {
  ...
  > {
    try {
      ...
      console.log(
        "🔍 Searching for payment slips for person:",
        personId,
        "by user:",
        user.id
      );
  ...
  async testDatabaseConnection(): Promise<{
    ...
    try {
      ...
      const { data: slipsData, error: slipsError } = await this.supabase
        .from("payment_slips")
        .select("id")
        .limit(1);
      ...
      return {
        success: true,
        message: `Database connection successful. User: ${
          user.email
        }, People: ${peopleData?.length || 0}, Slips: ${
          slipsData?.length || 0
        }`,
      };
    } catch (error) {
